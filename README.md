...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/v1/print` | Print a badge. Body: `{"name": "Anna Muster", "subtitle": "Guild42.ch"}` — `subtitle` is optional and falls back to the event currently selected in the kiosk's admin panel (or `DEFAULT_SUBTITLE` from `.env` until one is selected). `name` is cut to 15 characters, like in the kiosk. |
