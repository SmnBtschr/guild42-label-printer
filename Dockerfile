...
# (scripts/brother-setup.sh). This image runs the SAME code as a container - with PRINTER_SIM=1
# it needs no device at all and serves the animated simulator at /sim; with a device mapped in
# (--device /dev/usb/lp0) it would drive real hardware just like the Pi does.
#
